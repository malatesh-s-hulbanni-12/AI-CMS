...
const AddCreater = () => {
  const { axios } = useAppDevContext(); // <-- Using Developer Context
  ...
  const handleProgrammeSelect = useCallback(
    (programId) => {
      ...
      setFormData((p) => ({
        ...
        course: prog.department,
      }));
    },
    [programmes],
  );

  // ── SUBMIT VIA DEV ROUTE ──
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.email || !formData.password || !formData.name)
      return toast.error("Name, Email, and Password are required.");
    if (!formData.programId)
      return toast.error("Please select a specific programme.");
    ...
    try {
      ...
      // Hitting the secure dev route instead of the public register route
      const { data } = await axios.post("/api/dev/creators", payload);
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <FormInput
                ...
                type="email"
                icon={Mail}
                placeholder="faculty@gmit.edu.in"
                required
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input
                      disabled
                      value="Creator"
                      className="w-full pl-10 pr-4 py-2.5 bg-red-50 border border-red-100 rounded-lg text-[#BF1A1A] text-sm font-semibold cursor-not-allowed"
                    />
                  </div>
                </div>
                <div className="space-y-1">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    className="w-full pl-10 pr-8 py-2.5 border border-gray-300 bg-gray-50 focus:bg-white rounded-lg text-sm outline-none focus:ring-2 focus:ring-[#BF1A1A]"
                  >
